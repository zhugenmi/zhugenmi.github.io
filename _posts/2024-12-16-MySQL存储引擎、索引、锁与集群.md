---
title: MySQL存储引擎、索引、锁与集群
author: zhugenmi
date: 2024-12-16 10:00 +0800
...
- 查看：检测端口监听状况，Mycat的端口号是8066
  ...
  ![13](../assets/img/mysql/13.png)

- 连接：使用SQLYog连接Mycat

![14](../assets/img/mysql/14.png)

- 连接后显示：

  ![15](../assets/img/mysql/15.png)

##### 3.2 环境准备

- 配置模型

![16](../assets/img/mysql/16.png)

- 克隆虚拟机

![17](../assets/img/mysql/17.png)

![18](../assets/img/mysql/18.png)

![19](../assets/img/mysql/19.png)

![20](../assets/img/mysql/20.png)

![21](../assets/img/mysql/21.png)

![22](../assets/img/mysql/22.png)

![23](../assets/img/mysql/23.png)

- 修改配置网卡

  - 在第二个虚拟机中，生成全新mac地址

  ![24](../assets/img/mysql/24.png)

  ![25](../assets/img/mysql/25.png)
...
- 主服务器的配置
  ...
  // 创建用户
  CREATE USER 'imti'@'%' IDENTIFIED BY 'imti';
...
- 从服务器的配置
  ...
  // 执行
  change master to master_host='主服务器ip地址',master_port=3306,master_user='imti',master_password='imti',master_log_file='mysql-bin.000001',master_log_pos=4642;
  ```
...
- 在主服务器上修改server.xml
  - user标签主要用于定义登录mycat的用户和权限。如上面定义用户名mycat和密码123456，该用户可以访问的schema的DB逻辑库。
...
<mycat:schema xmlns:mycat="http://io.mycat/">
	...
	<dataHost name="localhost1" maxCon="1000" minCon="10" balance="1"
		...
		<writeHost host="hostM1" url="localhost:3306" user="root"
				   password="imti">
		<!-- 从服务器负责读操作 -->
		<readHost host="hostS1" url="192.168.203.135:3306" user="root" password="root" />
		</writeHost>
	</dataHost>
